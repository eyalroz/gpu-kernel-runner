...
cxxopts::Options basic_cmdline_options(const char* program_name)
{
    ...
    options.add_options()
        ...
        ("buffer-dimensions,image-dimensions,buffer-dims,image-dims,arg-dims,arg-dimensions", "Set one of the buffers' logical dimensions (1D ... 3D; mostly relevant for images; delimit dimensions with 'x', not ',')", cxxopts::value<std::vector<string>>())
        ("buffer-pitches,buffer-pitches-in-bytes,arg-pitches,arg-pitches-in-bytes,image-pitches,image-pitches-in-bytes", "Set the pitch for all-but-the-last dimensions of the buffer (for 2D, 3D; delimit pitches with 'x', not ',')", cxxopts::value<std::vector<string>>())
