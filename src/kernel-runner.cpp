...
cxxopts::Options basic_cmdline_options(const char* program_name)
{
    ...
    options.add_options()
        ...
        ("s,kernel-source-file,kernel-file,source-code,kernel-source-code,kernel-source", "Path to CUDA source file with the kernel function to compile; may be absolute or relative to the sources dir", cxxopts::value<string>())
