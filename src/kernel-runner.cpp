...
cxxopts::Options basic_cmdline_options(const char* program_name)
{
    ...
    options.add_options()
        ...
        ("O,compile-option,append-compile-option,append-compilation-option", "Append an arbitrary extra compilation option", cxxopts::value<std::vector<string>>())
        ...
        ("s,kernel-source-file,kernel-file,source-code,kernel-source-code,kernel-source,source-file", "Path to CUDA source file with the kernel function to compile; may be absolute or relative to the sources dir", cxxopts::value<string>())
