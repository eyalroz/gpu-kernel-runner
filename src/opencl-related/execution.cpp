...
void initialize_execution_context<execution_ecosystem_t::opencl>(execution_context_t& execution_context)
{
    ...
    execution_context.l2_cache_size = [&] {
        if (execution_context.options.clear_l2_cache) {
            constexpr const size_t opencl_device_max_cache_size_mibytes { 48 };
            spdlog::info("OpenCL does not support determining L2 cache size; assuming it is no larger than {} MiB", opencl_device_max_cache_size_mibytes);
