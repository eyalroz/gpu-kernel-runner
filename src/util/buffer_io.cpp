...
static std::vector<byte_type> read_verified_input_file_with_known_size(
    const filesystem::path& src,
    size_t known_file_size,
    ...
    bool zero_padding_data = false)
{
    ...
    try {
        ...
        std::vector<byte_type> result(target_buffer_size);
        auto known_file_size_ = static_cast<std::streamsize>(known_file_size);
        file.read(result.data(), known_file_size_);
        if (zero_padding_data) {
            std::fill(result.begin() + known_file_size_, result.end(), 0);
        } 
        return result;
        ...
        throw std::system_error(errno, std::generic_category(),
            "trying to read " + std::to_string(known_file_size) + " from file " + src.native());
    }
}
